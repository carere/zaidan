...
The four grouped tests exercised seven isolated fixture issues, including:
...
Integration testing also exposed a receipt/status race: a typed checkpoint written
while Docker inspection was in flight could be overwritten by a stopped-worker
failure. The coordinator now re-reads the final receipt after observing exit. A
real Docker test delays inspection until exit and verifies the checkpoint survives.
