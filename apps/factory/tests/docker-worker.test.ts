...
function fixture(number = 1, permits = false) {
  ...
  const workerOptions = {
    ...
    image:
      process.env.FACTORY_DOCKER_TEST_IMAGE ??
      (permits
        ? "zaidan-factory-worker-test-516:0.85.1"
        : number === 10
          ? "zaidan-factory-publication-test:0.85.1"
          : "zaidan-factory-worker-test:0.85.1"),
...
test("real Docker/Pi no-change classification preserves a triage proposal without candidate evidence", {
  ...
  try {
    ...
    assert.deepEqual(result.execution?.models, []);
