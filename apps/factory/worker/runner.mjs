...
try {
  ...
  const instructions = manifest.instructions
    ...
    .join("\n");
  const reevaluationInstructions = integration?.reevaluation
    ? `This is explicit reevaluation ${integration.reevaluation.id}. Read /input/request.json integration.reevaluation.issue.sourceContent in full: its current body, approved brief and specifications are authoritative for this phase, superseding the original manifest issue requirements. The reason is recorded in integration.reevaluation.reason. Update the assembled implementation to satisfy those current requirements and pass this scope to independent reviews. Preserve the original request.issue, resource snapshot, session and prior outputs as historical provenance. A reviewed old candidate is not evidence that these changed requirements are satisfied.`
    : "";
  ...
  const prompt = request.answer
    ...
    : acceptance && !resuming
      ...
      : integration && !resuming
        ? `/skill:${entry} Integrate the reviewed child ${integration.candidate.commit} into current graph head ${integration.expectedHead}. Resolve conflicts, checkpoint, validate the assembled result and invoke captured code-review with independent standards/spec delegates. ${reevaluationInstructions}`
        : request.phase > 0
