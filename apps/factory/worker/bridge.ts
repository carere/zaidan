...
export default function bridge(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    label: "Propose triage outcome",
    description:
      "After the original triage skill investigation, recommendation checkpoint, human direction and verification/grilling, propose the exact disclaimer-prefixed tracker outcome. This never writes to GitHub. The coordinator asks for approval and publishes authorized actions.",
    parameters: Type.Object({
      category: Type.Union([Type.Literal("bug"), Type.Literal("enhancement")]),
      state: Type.Union([
        Type.Literal("needs-triage"),
        Type.Literal("needs-info"),
        Type.Literal("ready-for-agent"),
        Type.Literal("ready-for-human"),
      ...
      wontfixReason: Type.Optional(
        Type.Union([Type.Literal("already-implemented"), Type.Literal("rejected")]),
      ),
      knowledge: Type.Optional(Type.Object({ path: Type.String(), content: Type.String() })),
    }),
    async execute(_id, args) {
      if (
        axis ||
        process.env.FACTORY_DELEGATE_RESULT ||
        request.issue.route !== "triage" ||
        !request.checkpoint?.answerId ||
        !request.answer
      )
        throw new Error(
          "Triage proposal requires original human direction in the admitted triage session",
        );
      if (!args.comment.startsWith("> *This was generated by AI during triage.*\n"))
        throw new Error("Triage comment requires the original disclaimer");
      const paths = [
        ...new Set(
          [
            ...git("diff", "--name-only", request.issue.startingRevision).split("\n"),
      ...
      for (const path of paths) {
        if (path === args.knowledge?.path) {
          if (readFileSync(`/state/checkout/${path}`, "utf8") !== args.knowledge.content)
            throw new Error("Rejection document differs from proposed content");
        } else if (/^(docs\/context\/root\.md|docs\/adr\/[0-9][a-z0-9.-]*\.md)$/.test(path))
          documents.push({ path, content: readFileSync(`/state/checkout/${path}`, "utf8") });
        else
          throw new Error(
            "Triage changed files outside domain/ADR/rejection documents; retain and reconcile before proposing publication",
          );
      }
      return outcome({
        type: "triage-proposed",
        proposal: { ...args, ...(documents.length ? { documents } : {}) },
      });
    },
  });
  pi.registerTool({
