...
const pagePath = fileURLToPath(new URL("./command.mdx", import.meta.url));
const demoNames = [
  "command-demo",
  "command-basic",
  "command-shortcuts",
...
describe("Command documentation", () => {
  it("links every approved focused demo to a default-export Solid module", async () => {
    ...
    expect(page).toContain("cmdk-solid");
    expect(page).toContain("https://github.com/create-signal/cmdk-solid#readme");
    expect(page).toContain("Kobalte Dialog");
    ...
    for (const name of demoNames) {
      ...
      expect(demo).toContain('from "@/registry/kobalte/ui/command"');
    ...
    expect(page).not.toContain("command-rtl");
