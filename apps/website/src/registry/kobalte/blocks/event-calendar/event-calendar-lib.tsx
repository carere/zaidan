 ...
 * segments clamped to the range. Rules: exclusive end - an event ending
 * exactly at zoned midnight emits NO segment for that day; zero-duration
 * events emit one min-height segment; allDay occurrences walk the same
 * absolute instants as timed ones and only drop startMin/endMin, so their
 * bounds have to already BE display-zone midnights (see CalendarEvent.allDay)
 * or the bar paints on the wrong days.
 */
function segmentOccurrence<TData>(
  ...
  let cursor: Date = zonedStartOfDay(clampStart, timeZone);
  ...
  for (const event of events) {
    ...
    if (custom) {
      for (let i = 0; i < custom.length; i++) {
        const occ = custom[i];
        if (replaced?.has(occ.start.getTime())) continue;
        if (!rangesIntersect({ start: occ.start, end: occ.end }, visibleRange)) continue;
        occurrences.push({
