...
interface BeginGestureConfig<TData> {
   ...
   * must never import from the chip module.
   */
  ui?: {
    dragCarry?: string;
...
function beginGesture<TData>(config: BeginGestureConfig<TData>) {
  ...
  const computeProposal = (
    ...
    if (kind === "create") {
      if (config.createAllDay || surface.columns.length === 0) {
        const anchor = zonedStartOfDay(config.createDay as Date, timeZone);
  ...
  // activation, transformed per pointermove, zero framework work per frame.
  ...
  // rejected (canDropEvent / bounds). Vanilla DOM: zero framework work per
  // frame; pairs with the not-allowed cursor and the ghost's destructive
  // marking.
...
function useEventCalendarGestures<TData = unknown>() {
  ...
  // Overlay classNames bridged from the view config into the vanilla-DOM
  // engine (which must never import from the chip module - circular).
  const ui = () => ({
    dragCarry: viewConfig.classNames?.dragCarry,
    dragCarryInvalid: viewConfig.classNames?.dragCarryInvalid,
    dropHint: viewConfig.classNames?.dropHint,
  });
  ...
  const beginResize = (
    ...
    if (!canResize(segment)) {
      e.stopPropagation();
      beginBlockedGesture(instance, e, segment, "resize");
      return;
    }
    e.stopPropagation();
    e.preventDefault();
  ...
  const beginCreate = (e: PointerEvent, day: Date, allDay: boolean) => {
    if (e.button !== 0) return;
