 ...
 * selectors and one-direction margins are what make the guarantee hold.
 *
 * Ported from shadcn/ui `packages/tests/src/tests/typeset.test.ts` so the
 * portable stylesheet keeps the same guarantee as upstream. Only the portable
 * file is covered: `src/styles/typeset.css` holds docs-site overrides that are
 * never shipped to consumers.
 */
const read = (file: string) =>
  readFileSync(path.resolve(__dirname, `../registry/kobalte/styles/${file}`), "utf-8");
...
describe.each(Object.entries(FILES))("%s append-stability contract", (file, contents) => {
  ...
  it("only spaces forward: no margin-bottom, margin-block-end always 0", () => {
    ...
    const ends = [...contents.matchAll(/margin-block-end: ([^;]+);/g)].map((match) => match[1]);
