...
function getFileIcon(language: string) {
  ...
  if (language === "json") {
    return h("svg", iconProps, [h("path", { d: jsonIconPath })]);
  }

  if (["js", "jsx", "ts", "tsx", "typescript"].includes(language)) {
    return h("svg", iconProps, [h("path", { d: typescriptIconPath })]);
  }

  if (language === "css") {
    return h("svg", iconProps, [h("path", { d: cssIconPath })]);
  }

  return h("svg", { ...iconProps, fill: "none", stroke: "currentColor", strokeWidth: 2 }, [
    ...fileIconPath.map((d) => h("path", { d })),
  ]);
}
...
try {
  (() => {
    const maxCollapsedHeight = 256;

    function updateFrame(frame, expanded) {
      frame.dataset.expanded = String(expanded);
      frame.querySelectorAll("[data-code-collapse-toggle]").forEach((button) => {
        button.setAttribute("aria-expanded", String(expanded));
        const label = button.querySelector("[data-code-collapse-label]");
        if (label) label.textContent = expanded ? "Collapse" : "Expand";
      });
    }

    function initialize(root) {
      root.querySelectorAll?.(".expressive-code .frame[data-code-collapse-candidate]").forEach((frame) => {
        if (frame.dataset.codeCollapseInitialized) return;

        const pre = frame.querySelector("pre");
        if (!pre) return;

        frame.dataset.codeCollapseInitialized = "true";
        if (pre.scrollHeight <= maxCollapsedHeight + 1) return;

        frame.dataset.codeCollapsible = "true";
        frame.querySelectorAll("[data-code-collapse-control]").forEach((control) => {
          control.hidden = false;
        });
        frame.querySelectorAll("[data-code-collapse-toggle]").forEach((button) => {
          button.addEventListener("click", () => {
            updateFrame(frame, frame.dataset.expanded !== "true");
          });
        });
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => initialize(node));
      });
    });
    observer.observe(document.body, { childList: true, subtree: true });
    document.addEventListener("astro:page-load", () => initialize(document));
  })();
} catch (error) {
  console.error("[EC] titled-code-blocks failed:", error);
}
`;

export function pluginTitledCodeBlocks(): ExpressiveCodePlugin {
  return {
    name: "Titled code blocks",
    jsModules: [collapseModule],
    hooks: {
      postprocessRenderedBlock({ codeBlock, renderData }) {
        const { blockAst } = renderData;
        const header = select("figcaption.header", blockAst);
        const pre = select("pre", blockAst);
        const title = select(".title", blockAst);

        if (!header || !pre || !title) return;

        header.properties["data-language"] = codeBlock.language;
        header.children.unshift(getFileIcon(codeBlock.language));
        blockAst.properties["data-code-collapse-candidate"] = "";

        const topControl = h(
          "div",
          { className: "code-collapse-control", hidden: true, "data-code-collapse-control": "" },
          [
            h(
              "button",
              {
            ...
            h("span", { className: "code-collapse-separator", "aria-hidden": "true" }),
          ],
        );
        const bottomControl = h(
          "button",
          {
            type: "button",
        ...
        blockAst.children.push(bottomControl);
      },
    },
  };
}
