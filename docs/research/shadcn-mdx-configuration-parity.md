...
Zaidan also has behavior absent from the Fumadocs default preset: frontmatter handling for its current compiler, GitHub-alert/directive transforms, `kbd` classes, raw HTML, autolinked headings, build-time component source injection, and `file=` code imports ([active compiler](../../apps/website/src/lib/vite-plugins/mdx.ts#L54-L108)). Those plugins must either remain or their content syntax/renderers must be migrated.
