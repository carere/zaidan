...
Source: https://tanstack.com/router/latest/docs/framework/solid/guide/search-params
...
> ⚠️ Warning: Router expects the valibot 1.0 package to be installed.
...
> ⚠️ Warning: Router expects the arktype 2.0-rc package to be installed.
...
If the search for the current page shall be updated and the `from` prop is specified, the `to` prop can be omitted. Here's an example:
...
In this scenario you can set `to="."` which will give you access to loosely typed search params. Here is an example that illustrates this:
