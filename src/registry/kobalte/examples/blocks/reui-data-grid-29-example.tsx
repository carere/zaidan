...
import { MoreHorizontalIcon, RefreshCwIcon } from "lucide-solid";
...
export default function Pattern() {
  ...
  const columns: ColumnDef<IData>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <span class="text-foreground">{row.original.role}</span>,
      size: 160,
      enableSorting: true,
    },
    {
      ...
      <Card class="w-full gap-0 p-0">
        <CardHeader class="flex items-center justify-between px-3 py-2">
          <div class="flex items-center gap-2">
            <span class="font-medium text-foreground text-sm">Team Members</span>
            {(rowPinning().top?.length ?? 0) > 0 && (
              <Badge variant="outline">{rowPinning().top?.length} pinned</Badge>
            )}
          </div>
