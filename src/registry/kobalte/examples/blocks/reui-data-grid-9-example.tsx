...
import { SquareMinusIcon, SquarePlusIcon } from "lucide-solid";
...
import { DataGridTable } from "@/registry/kobalte/blocks/reui-data-grid/data-grid-table";
...
export default function Pattern() {
  ...
  const [sorting, setSorting] = createSignal<SortingState>([]);
  const [expandedRows, setExpandedRows] = createSignal<ExpandedState>({});
  ...
  const columns: ColumnDef<Data>[] = [
    {
      ...
      cell: ({ row }) => {
        return row.getCanExpand() ? (
          ...
          >
            {row.getIsExpanded() ? (
              <SquareMinusIcon class="size-3.5" />
            ) : (
              <SquarePlusIcon class="size-3.5" />
            )}
          </Button>
        ) : null;
      },
        ...
        return (
          <div class="flex items-center gap-3">
            ...
            <div class="space-y-px">
              <div class="font-medium text-foreground">{row.original.customer}</div>
              <div class="text-muted-foreground">{row.original.customerEmail}</div>
            </div>
          </div>
        );
      },
    ...
    },
    {
      ...
      cell: (info) => {
        ...
        return (
          <button
            type="button"
            class="cursor-pointer font-medium text-foreground text-sm hover:text-primary"
