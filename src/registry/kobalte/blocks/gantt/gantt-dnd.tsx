...
import type { GanttInstance } from "./gantt";
import { resolveScheduleMode, useGantt, useGanttViewConfig } from "./gantt";
...
/** A pointer event as Solid delivers it to a JSX handler. */
type GanttPointerEvent = PointerEvent & { currentTarget: HTMLElement };
 ...
 * gesture on cleanup so window listeners, body-appended overlays and the
 * gantt-dragging body class never outlive the gantt.
 */
function useGanttGestureTeardown(): void {
  onCleanup(cancelActiveGanttGestures);
}
...
function beginGesture<TData>(config: BeginGestureConfig<TData>) {
   ...
   * class="text-sm") rescales the whole gantt, drag clone included.
   */
  ...
  const clampToNeighbours = (start: Date, end: Date): { start: Date; end: Date } => {
    ...
    if (floor === Number.NEGATIVE_INFINITY && ceiling === Number.POSITIVE_INFINITY) {
      return { start, end };
    }
  ...
  // the pointer's x via transform (no per-frame rendering), lifted with a shadow.
  ...
  const cleanup = () => {
    ...
    // consumer-rendered overlays are owned by the view: they unmount when the
    // drag state clears, so the engine must never remove them itself
...
interface GanttGestures<TData = unknown> {
  beginMove: (e: GanttPointerEvent, segment: GanttSegment<TData>) => void;
  beginResize: (e: GanttPointerEvent, segment: GanttSegment<TData>, edge: "start" | "end") => void;
  beginCreate: (e: GanttPointerEvent) => void;
...
function useGanttGestures<TData = unknown>(): GanttGestures<TData> {
  ...
  const canDrag = (segment: GanttSegment<TData>) => {
  ...
  const canResize = (segment: GanttSegment<TData>) => {
    const { interactions } = instance.getState();
    const event = segment.occurrence.event;
    return interactions.resize && !event.readOnly && event.resizable !== false;
  };

  const beginMove = (e: GanttPointerEvent, segment: GanttSegment<TData>) => {
    ...
    beginGesture({
      ...
      startEvent: e,
      segment,
      // presence flag only: the engine skips its default overlay DOM and
      // positions the consumer-rendered node instead
      customMoveOverlay: !!viewConfig.renderDragPreview,
  ...
  const beginResize = (
    ...
    beginGesture({
      ...
      startEvent: e,
      segment,
      customResizeOverlay: !!viewConfig.renderResizeIndicator,
      scheduleMode: viewConfig.scheduleMode,
    });
  };

  const beginCreate = (e: GanttPointerEvent) => {
...
export type { GanttGestures, GanttPointerEvent };
export {
