...
const pagePath = fileURLToPath(new URL("./pagination.mdx", import.meta.url));
const demoNames = ["pagination-demo", "pagination-simple", "pagination-icons-only"];
...
describe("Pagination documentation", () => {
  it("links every approved focused demo to a default-export Solid module", async () => {
    ...
    expect(page).not.toContain("pagination-rtl");
