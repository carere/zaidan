...
const pagePath = fileURLToPath(new URL("./table.mdx", import.meta.url));
const demoNames = ["table-demo", "table-footer", "table-actions"];
...
describe("Table documentation", () => {
  it("links every approved focused demo to a default-export Solid module", async () => {
    ...
    expect(page).not.toContain("table-rtl");
