...
const pagePath = fileURLToPath(new URL("./scroll-area.mdx", import.meta.url));
const demoNames = ["scroll-area-demo", "scroll-area-horizontal-demo"];
...
describe("Scroll Area documentation", () => {
  it("links every approved focused demo to a default-export Solid module", async () => {
    ...
    expect(page).not.toContain("scroll-area-rtl");
