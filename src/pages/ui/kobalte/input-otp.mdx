...
Use the `pattern` prop to define a custom regex pattern for the OTP input. Pass `null` to allow all characters.
...
A common pattern for PIN codes. This uses the `pattern="^\d*$"` prop.

<ComponentPreview name="input-otp-four-digits" />

## Alphanumeric

Use `pattern="^[a-zA-Z0-9]*$"` to accept both letters and numbers.
