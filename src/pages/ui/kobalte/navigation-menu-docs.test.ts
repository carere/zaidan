...
const pagePath = fileURLToPath(new URL("./navigation-menu.mdx", import.meta.url));
const demoNames = ["navigation-menu-demo"];

describe("Navigation Menu documentation", () => {
  it("links every approved focused demo to a default-export Solid module", async () => {
    const page = await readFile(pagePath, "utf8");

    expect(page).toContain("Kobalte Navigation Menu");
    expect(page).toContain(
      "https://kobalte.dev/docs/core/components/navigation-menu#api-reference",
    );
    ...
    for (const name of demoNames) {
      ...
      expect(demo).toContain('from "@/registry/kobalte/ui/navigation-menu"');
      ...
      expect(demo).toContain("/docs/components/kobalte/alert-dialog");
    ...
    expect(page).not.toContain("navigation-menu-rtl");
