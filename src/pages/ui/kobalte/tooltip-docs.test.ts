...
const pagePath = fileURLToPath(new URL("./tooltip.mdx", import.meta.url));
const demoNames = ["tooltip-demo", "tooltip-sides", "tooltip-keyboard", "tooltip-disabled"];

describe("Tooltip documentation", () => {
  it("links every approved focused demo to a default-export Solid module", async () => {
    const page = await readFile(pagePath, "utf8");

    expect(page).toContain("Kobalte Tooltip");
    expect(page).toContain("https://kobalte.dev/docs/core/components/tooltip#api-reference");
    ...
    expect(page).not.toContain("tooltip-rtl");
