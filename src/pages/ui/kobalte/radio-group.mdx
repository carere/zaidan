...
The Radio Group component follows the accessible radio-group semantics of [Kobalte Radio Group](https://kobalte.dev/docs/core/components/radio-group). Its Zaidan wrapper is implemented with native Solid primitives to preserve the pinned Base UI value, form, and cancellation contract.
...
Use the `disabled` prop on `RadioGroupItem` to disable an individual item.
...
For primitive props and keyboard behavior, see the [Kobalte Radio Group API reference](https://kobalte.dev/docs/core/components/radio-group#api-reference).
