...
- The "Add Filter" menu uses the dropdown primitives’ keyboard navigation. Opening by keyboard focuses the search input when present. Type to filter, then press <kbd>↓</kbd> / <kbd>↑</kbd> to move focus into the results. On a field, <kbd>→</kbd> opens its submenu; <kbd>←</kbd> from an option returns focus to its trigger, and <kbd>Esc</kbd> closes the menu.
- In a submenu, <kbd>↓</kbd> / <kbd>↑</kbd> move between options and <kbd>Enter</kbd> / <kbd>Space</kbd> select or toggle the focused option. Virtualized lists must keep the highlighted row mounted to preserve focus. Existing filter-value editors use a combobox with `aria-activedescendant`.
