...
1. Run Steps 1-7 yourself (Resolve Source, Auto-Detect, Dependency Pre-Flight, Visual Analysis, User Story Generation, Research Primitives)
...
3. Spawn one `builder` subagent per file (or per small group) via the Task tool with:
   ...
   - The shared PRIMITIVE, and primitive research from Step 7
...
1.1 - Fetch the source file from `SOURCE_URL`:
```bash
curl -s "{SOURCE_URL}"
```

1.2 - If the response is empty or 404, abort:
```
RESULT: FAILURE | Component: <COMPONENT_NAME> | Primitive: <PRIMITIVE> | Error: Source not found at {SOURCE_URL}
```

1.3 - Parse the fetched source to determine:
...
1.4 - If `REGISTRY_URL` points to a JSON registry, fetch it and extract metadata for this component (dependencies, registryDependencies, cssVars).

1.5 - Store the resolved data for use in subsequent steps.
...
### Step 3: Dependency Pre-Flight (HARD GATE)

This step is a HARD GATE. If ANY registry dependencies are missing, the transformation MUST abort.

3.1 - Using the registry dependencies identified in Step 2, gather the list of required registry dependencies.

3.2 - Read the target primitive's registry: `src/registry/<PRIMITIVE>/registry.json`

3.3 - For each entry in `registryDependencies`, check if a matching `name` exists in the registry's items.

3.4 - **If ALL dependencies are found**: Log "Dependency gate: PASS (N/N dependencies found)" and proceed to Step 5.

3.5 - **If ANY dependencies are missing**: Output the following and STOP immediately. Do NOT proceed to transformation.
...
3.6 - Map third-party dependencies (from the `dependencies` array) using the `react-to-solid` skill's dependency mapping table:
...
4.2 - Spawn a `qa` via the Task tool:
  - `subagent_type: "qa"`
  - Prompt: the prompt built in 4.1
  - Wait for the agent to complete

4.3 - Store the agent's output (interaction log + screenshot paths) as `VISUAL_ANALYSIS_OUTPUT`.
...
4.4 - If the agent fails or times out, output the following and STOP. Do NOT proceed to Step 6 or any subsequent steps:
...
5.1 - Determine the output path: `ai_review/user_stories/<COMPONENT_NAME>.yaml`.

5.2 - Determine the component URL based on the auto-detected type from Step 3:
...
5.3 - If `VISUAL_ANALYSIS_OUTPUT` is non-empty, derive workflow steps from the qa output:
  ...
  - **Critical**: The first step of EVERY story workflow MUST be: `Navigate to <component URL from 5.2>`

5.4 - Replace `<ComponentTitle>` with the PascalCase title (e.g., "Dialog", "Alert Dialog").

5.5 - Write the YAML file to the output path. The file must use the `stories` root array structure.
...
### Step 7: File Classification + Intent Translation
...
**7.1 — Classify**: Scan the file for React markers:
...
**7.2 — If PASSTHROUGH** (no React markers found): Copy the file as-is to the output path. Only fix import paths:
...
**7.3 — If INTENT TRANSLATION** (React markers found):
...
**7.4 — Preserve all `data-slot` attributes** from the source exactly as they appear.

**7.5 — Preserve all CSS/Tailwind classes** — only change `className` to `class`, do not modify class values.

**7.6 — Map Base UI data attributes and CSS variables** to Kobalte/Base-UI/Corvu equivalents using mapping tables from the `react-to-solid` skill, plus component-specific mappings from Step 7.
...
9.2 - Grep for remaining React patterns (must find NONE):
```bash
# Component mode:
...
### Step 10: Auto-Install Dependencies

After validation passes, install any new dependencies that the transformed code requires.

10.1 - Collect all `import` statements from the transformed file(s):
...
10.2 - Extract external package names from imports:
...
10.3 - Read `package.json` to get currently installed packages (both `dependencies` and `devDependencies`)

10.4 - For each extracted package NOT already in `package.json`:
...
10.5 - Record the list of newly installed packages for Step 11.
